...
This script loads validation/test annotation files and visualizes the 
ground truth keypoints on the original images.

Usage:
    python scripts/visualize_gt_annotations.py --split val --num-samples 20
    python scripts/visualize_gt_annotations.py --split test --data-split split1
"""
...
def build_annotation_map(data):
    ...
    # Map category_id to category info
...
def visualize_annotation(img_path, img_info, annotations, categories, output_path):
    ...
    # Create figure with subplots for each annotation
    ...
    for idx, (ax, ann) in enumerate(zip(axes, annotations)):
        ...
        # Draw skeleton edges first (if available)
        if skeleton:
...
def main():
    ...
    sorted_cats = sorted(cat_counts.items(), key=lambda x: x[1], reverse=True)
    for cat_id, count in sorted_cats[:10]:  # Top 10
