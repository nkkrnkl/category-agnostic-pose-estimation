...
def train_one_epoch(
    ...
    **kwargs,
    ...
    print_freq = 10  # Reduced verbosity: print every 100 iterations instead of 10
    ...
    for batched_inputs, batched_extras in metric_logger.log_every(
        ...
        gt_instances = [
            x["instances"].to(device) if x["instances"] is not None else None
        ...
        # Only log essential metrics for cleaner output
        # Extract key losses instead of logging all intermediate decoder losses
        essential_metrics = {
            "loss": loss_value,
            "loss_ce": loss_dict_scaled.get("loss_ce", 0),
            "loss_coords": loss_dict_scaled.get("loss_coords", 0),
        }
        metric_logger.update(**essential_metrics)
    ...
    for batched_inputs, batched_extras in metric_logger.log_every(
        ...
        for i in range(bs):
            if dataset_name == "stru3d":
                ...
                gt_polys = [
                    x[0].reshape(-1, 2).astype(np.int32)
                    for x in gt_instances[i].gt_masks.polygons
                ]
                ...
                for gt_poly, gt_id in zip(
                    gt_instances[i].gt_masks.polygons,
                    gt_instances[i].gt_classes.detach().cpu().tolist(),
                ):
        ...
        if plot_density and len(room_polys) > 0:
            pred_room_map = plot_density_map(
                samples[i], image_size, room_polys, pred_room_label_per_scene
            )
            cv2.imwrite(
                os.path.join(
                    output_dir, "{}_pred_room_map_{}.png".format(scene_ids[i], epoch)
                ),
                pred_room_map,
            )
    ...
    for batched_inputs, batched_extras in metric_logger.log_every(
        ...
        for i in range(bs):
            ...
            if dataset_name == "stru3d":
                ...
                curr_data_rw = S3DRW(curr_opts, mode="online_eval")
                evaluator = Evaluator(
                    curr_data_rw, curr_opts, disable_overlapping_filter=True
                )
            elif dataset_name == "scenecad":
                gt_polys = [
                    gt_instances[i]
                ...
                evaluator = Evaluator_RPlan(
                    disable_overlapping_filter=True, wd_as_line=wd_as_line
                )
                    ...
                    if per_token_sem_loss:
                        pred_classes, counts = np.unique(
                            pred_room_label_per_scene[
                                start_poly_indices[j] : start_poly_indices[j + 1]
                            ][:-1],
                            return_counts=True,
                        )
    ...
    for batched_inputs, batched_extras in data_loader:
        ...
        for i in range(pred_logits.shape[0]):
            ...
            if "pred2gt_indices" in quant_result_dict_scene:
                ...
                room_polys, gt_room_polys, pred_mask, gt_mask = (
                    sort_polygons_by_matching(
            ...
            prec, rec = (
                quant_result_dict_scene["room_prec"],
                quant_result_dict_scene["room_rec"],
            )
            ...
            if plot_pred:
                gt_floorplan_map = plot_floorplan_with_regions(
                    gt_room_polys,
                    matching_labels=gt_mask,
                    base_scale=image_size,
                    scale=1024,
                )
                floorplan_map = plot_floorplan_with_regions(
                    room_polys,
                    matching_labels=pred_mask,
                    base_scale=image_size,
                    scale=1024,
                )

                concatenated_map = concat_floorplan_maps(
                    gt_floorplan_map, floorplan_map, plot_statistics
                )
                cv2.imwrite(
                    os.path.join(
                        output_dir, "{}_pred_floorplan.png".format(scene_ids[i])
                    ),
                    concatenated_map,
                )
            ...
            if save_pred:
                ...
                json_path = os.path.join(
                    output_dir, "jsons", "{}.json".format(str(scene_ids[i]).zfill(5))
                )
                os.makedirs(os.path.dirname(json_path), exist_ok=True)
                polys_list = [poly.astype(float).tolist() for poly in room_polys]
                if semantic_rich:
                    polys_list += [
                        window_door.astype(float).tolist()
                        for window_door in window_doors
                    ]
                ...
                json_result_path = os.path.join(
                    output_dir,
                    "result_jsons",
                    "{}.json".format(str(scene_ids[i]).zfill(5)),
                )
                new_quant_result_dict_scene = compute_f1(
                    copy.deepcopy(quant_result_dict_scene), metric_category
                )
            ...
            if plot_density:
                ...
                gt_room_map = plot_density_map(
                    samples[i], image_size, gt_polys, gt_polys_types, plot_text=False
                )

                concatenated_map = concat_floorplan_maps(
                    gt_room_map, pred_room_map, plot_statistics
                )
                cv2.imwrite(
                    os.path.join(
                        output_dir, "{}_pred_room_map.png".format(scene_ids[i])
                    ),
                    concatenated_map,
                )
    ...
    for batched_inputs, batched_extras in data_loader:
        ...
        for i in range(bs):
            ...
            if dataset_name == "stru3d":
                ...
                evaluator = Evaluator_RPlan(
                    disable_overlapping_filter=True,
                    iou_thres=iou_thres,
            ...
            for j, poly in enumerate(all_room_polys):
                ...
                if not semantic_rich:
                    ...
                    if per_token_sem_loss:
                        ...
                        pred_class = pred_classes[np.argmax(counts)]
                        pred_logit = pred_room_logit_per_scene[
                            start_poly_indices[j] : start_poly_indices[j + 1]
                        ][:-1]
                    else:
                        pred_class = pred_room_label_per_scene[
                    ...
                    if wd_as_line:
                        ...
                        # window / door
                            ...
                            if pred_class not in door_window_index:
                                wd_prob = np_softmax(
                                    pred_logit[:, door_window_index].sum(0)
                                )
            ...
            if plot_pred:
                ...
                if merge:
                    ...
                    cv2.imwrite(
                        os.path.join(
                            output_dir, "{}_gt_floorplan.png".format(scene_ids[i])
                        ),
                        gt_floorplan_map,
                    )
                ...
                if semantic_rich:
                    ...
                    floorplan_map = plot_semantic_rich_floorplan_opencv(
                        ...
                        os.path.join(
                            output_dir, "{}_pred_floorplan_sem.png".format(scene_ids[i])
                        ),
            ...
            if plot_gt:
                ...
                cv2.imwrite(
                    os.path.join(output_dir, "{}_gt.png".format(scene_ids[i])), gt_image
                )
            ...
            if plot_density:
                ...
                if merge:
                    ...
                    cv2.imwrite(
                        os.path.join(
                            output_dir, "{}_gt_room_map.png".format(scene_ids[i])
                        ),
                        gt_room_map,
                    )
...
def concat_floorplan_maps(gt_floorplan_map, floorplan_map, plot_statistics={}):
    ...
    padding = np.full(
        (gt_floorplan_map.shape[0], 10, gt_floorplan_map.shape[2]),
        pad_color,
        dtype=np.uint8,
    )
    ...
    top_padding = np.full(
        (100, concatenated_map.shape[1], concatenated_map.shape[2]),
        pad_color,
        dtype=np.uint8,
    )
    ...
    font_scale = 1
    font_color = (
        (255, 255, 255) if gt_floorplan_map.shape[2] == 3 else (0, 0, 255, 255)
    )  # White text
    thickness = 2
