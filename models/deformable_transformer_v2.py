...
def Embedding(num_embeddings, embedding_dim, padding_idx=None, zero_init=False):
    """
    Create embedding layer with custom initialization.
...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8,
        ...
        Args:
            ...
            poly_refine: Whether to use polygon refinement
            ...
            dec_n_points: Number of decoder sampling points
            enc_n_points: Number of encoder sampling points
            query_pos_type: Query position type
            vocab_size: Vocabulary size
            seq_len: Sequence length
            pre_decoder_pos_embed: Whether to use pre-decoder positional embedding
            learnable_dec_pe: Whether to use learnable decoder positional embedding
            dec_attn_concat_src: Whether to concatenate source in decoder attention
            dec_qkv_proj: Whether to use QKV projection in decoder
            dec_layer_type: Decoder layer type
    ...
    def _create_causal_attention_mask(self, seq_len):
        """
        Create a causal attention mask for a sequence.
    ...
    def _setup_caches(self, max_batch_size, max_seq_length, max_vision_length, model_dim, nhead, dtype, device):
        """
        Setup KV caches for autoregressive generation.
        
        Args:
            max_batch_size: Maximum batch size
            max_seq_length: Maximum sequence length
            max_vision_length: Maximum vision length
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        Args:
            ...
            use_qkv_proj: Whether to use QKV projection
        """
        super().__init__()
        self.d_model = d_model
        if use_qkv_proj:
...
class TransformerDecoderLayerV5(nn.Module):
    """
    Transformer decoder layer V5 with average pooling for image features.
    """
    def __init__(self, d_model=256, d_ffn=1024,
                 dropout=0.1, activation="relu",
                 n_levels=4, n_heads=8, n_points=4, use_qkv_proj=True):
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask=None, 
        ...
        if attn_concat_src:
            ...
            pool_src = torch.cat(pool_src_list, dim=1)
...
class TransformerDecoderLayerV6(nn.Module):
    """
    Transformer decoder layer V6 with average pooling for image features.
    """
    def __init__(self, d_model=256, d_ffn=1024,
                 dropout=0.1, activation="relu",
                 n_levels=4, n_heads=8, n_points=4, use_qkv_proj=True):
...
class TransformerDecoderLayerV4(nn.Module):
    """
    Transformer decoder layer V4 with custom sampling.
    """
    def __init__(self, d_model=256, d_ffn=1024,
                 dropout=0.1, activation="relu",
                 n_levels=4, n_heads=8, n_points=4, use_qkv_proj=True):
        """
        Initialize decoder layer V4.
...
class TransformerDecoderLayerV41(nn.Module):
    """
    Transformer decoder layer V41 with deformable points.
    """
    def __init__(self, d_model=256, d_ffn=1024,
                 dropout=0.1, activation="relu",
                 n_levels=4, n_heads=8, n_points=4, use_qkv_proj=True):
        """
        Initialize decoder layer V41.
...
class TransformerDecoderLayerV3(nn.Module):
    """
    Transformer decoder layer V3 with BiXAttn.
    """
    def __init__(self, d_model=256, d_ffn=1024,
                 dropout=0.1, activation="relu",
                 n_levels=4, n_heads=8, n_points=4, is_last_layer=False, **kwargs):
        """
        Initialize decoder layer V3.
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_flatten, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        output = self._seq_embed(seq11=seq_kwargs['seq11'], seq12=seq_kwargs['seq12'], 
                                ...
                                delta_y1=seq_kwargs['delta_y1'], delta_y2=seq_kwargs['delta_y2'])
        if decode_token_pos is not None:
...
def build_deforamble_transformer(args, pad_idx=None):
    """
    Build deformable transformer from arguments.
    ...
    Returns:
        DeformableTransformer instance
    """
