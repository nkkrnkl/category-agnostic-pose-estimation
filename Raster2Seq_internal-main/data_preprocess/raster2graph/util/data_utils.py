...
def data_to_cuda(samples, targets):
    return samples.to(torch.device('cuda')), [{k: v if isinstance(v, str) else v.to(torch.device('cuda')) for k, v in t.items()} for t in targets]
...
def draw_given_layers_on_tensors_random_region(given_layers, tensors, graphs):
    ...
    for i in range(len(given_layers)):
        ...
        for j, pos in enumerate(given_points):
            ...
            valid_yellow_pos_down = int(pos[1] + rectangle_radius) if (pos[1] + rectangle_radius) < temp_tensor.shape[2] else temp_tensor.shape[
                                                                                                                               2] - 1
            valid_yellow_pos_left = int(pos[0] - rectangle_radius) if (pos[0] - rectangle_radius) >= 0 else 0
            valid_yellow_pos_right = int(pos[0] + rectangle_radius) if (pos[0] + rectangle_radius) < temp_tensor.shape[1] else temp_tensor.shape[
                                                                                                                                1] - 1
